    ...
    if profile.is_system:
        ...
        for e in items:
            ...
            channels_data.append({
                ...
                'play_url': e.channel.stream_url if getattr(e.channel, 'keep_original_link', False) else f"/api/channels/play/{e.channel.id}",
                'play_links': {
                    'original': e.channel.stream_url,
                    'track': e.channel.stream_url if getattr(e.channel, 'keep_original_link', False) else f"/api/channels/track/{e.channel.id}",
                    'smart': e.channel.stream_url if getattr(e.channel, 'keep_original_link', False) else f"/api/channels/play/{e.channel.id}",
                    'ts': e.channel.stream_url if getattr(e.channel, 'keep_original_link', False) else f"/api/channels/play/{e.channel.id}",
                    'hls': e.channel.stream_url if getattr(e.channel, 'keep_original_link', False) else f"/api/channels/hls-manifest/{e.channel.id}/index.m3u8",
                },
                'keep_original_link': getattr(e.channel, 'keep_original_link', False),
